 ...
 * @brief returns whether the circular array is empty of data
 ...
 * @brief buf_full returns whether the circular array is full of unprocessed data or not
 * @param arr a pointer to a circular array object
 * @return
 */
 ...
 * @param arr a pointer to a circular array object
 * @return 1 success, 0 on circ array empty
 */
uint8_t buf_getbyte(CircArr_InitTypeDef* arr, uint8_t* bytebuff)
{
if ( buf_empty(arr) ) {
	return 0;
}
*bytebuff = arr->buf[arr->n_r++ % arr->size];
return 1;
}
...
{
	uint8_t ret = 1;
	if ( buf_full(arr) ) {//If buff is full, we should loboff oldest element
	arr->n_r++;
	ret = 0;
	} //increment n_r
